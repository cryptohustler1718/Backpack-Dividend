const stocks={
  SPCX:{name:'SpaceX',price:100,dividend:0,frequency:'None',note:'Tokenized security · demo',eligible:true},
  AAPL:{name:'Apple',price:200,dividend:1,frequency:'Quarterly',note:'Traditional + tokenized · demo',eligible:false},
  NVDA:{name:'NVIDIA',price:120,dividend:.1,frequency:'Quarterly',note:'Traditional + tokenized · demo',eligible:false}
};
...
function equalize(){const share=100/selected.length;selected.forEach(t=>allocations[t]=share)}
...
function renderAllocationInputs(){
  ...
  $$('[data-allocation]').forEach(i=>i.addEventListener('input',()=>{const t=i.dataset.allocation,other=selected.find(x=>x!==t);allocations[t]=Number(i.value);if(other){allocations[other]=100-allocations[t];const otherInput=document.querySelector(`[data-allocation="${other}"]`);if(otherInput)otherInput.value=allocations[other];document.querySelector(`[data-allocation-value="${other}"]`).textContent=Math.round(allocations[other])+'%'}document.querySelector(`[data-allocation-value="${t}"]`).textContent=Math.round(allocations[t])+'%';update()}));
}
...
function update(){
  ...
  const totalDiv=basket.reduce((a,x)=>a+x.div,0),priceValue=amount*(1+shock/100),finalValue=priceValue+totalDiv;
